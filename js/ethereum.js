...
web3.version.getNetwork(function(err, version){
  //config
  var testnet = true;
  var contractAddr = testnet ? '0x8cdaf0cd259887258bc13a92c0a6da92698644c0' : '0xaf30d2a7e90d7dc361c8c4585e9bb7d2f6f15bc7';
  ...
  function updateDisplay() {
    ...
    var link = 'http://127.0.0.1:8545/token/'+contractAddr;
      ...
      if (!data || data=='') {
        ...
        send(address, pk, undefined, undefined, data, amount, 300000, function(err, result){
          if (err) {
            ...
            var link = 'http://localhost:9545/tx/'+txHash;
  ...
  function getBalance(addr, callback) {
    ...
    var url = 'https://localhost:9545/api?module=account&action=balance&address='+addr+'&tag=latest';
  ...
  function getTokenBalance(addr, callback) {
    ...
    var url = 'https://localhost:9545/api?module=proxy&action=eth_call&to='+contractAddr+'&data='+data+'&tag=latest';
    $.get(url, function(data){
      if (data.result!='Error!') {
        callback(null, data.result);
      } else {
  ...
  function getBlockNumber(callback) {
    var url = 'https://localhost:9545/api?module=proxy&action=eth_blockNumber';
    $.get(url, function(data){
      if (data.result!='Error!') {
        callback(null, web3.toDecimal(data.result));
      } else {
  ...
  function getNextNonce(addr, callback) {
    var url = 'https://localhost:9545/api?module=proxy&action=eth_GetTransactionCount&address='+addr+'&tag=latest';
    $.get(url, function(data){
      if (data.result!='Error!') {
        callback(null, data.result);
      } else {
  ...
  function loadLogs(callback) {
    var fromBlock = 0;
    var toBlock = 'latest';
    var url = 'https://localhost:9545/api?module=logs&action=getLogs&address='+contractAddr+'&fromBlock='+fromBlock+'&toBlock='+toBlock;
          ...
          function proxySend() {
            try {
              ...
              getNextNonce(addr, function(err, nextNonce){
                ...
                var url = 'https://localhost:9545/api';
  ...
  function confirm(txHash, callback) {
    var url ='https://localhost:9545/api?module=proxy&action=eth_getTransactionReceipt&txhash='+txHash;
    var confirmed = false;
    async.until(
      function() {
        return confirmed;
